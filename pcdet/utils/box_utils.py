...
def points_in_box_3d(points, boxes):
    """
    检查点是否在3D框内
    Args:
        points: (N, 3) 点坐标数组
        boxes: (M, 7) 或 (7,) 3D框数组，格式为 [x, y, z, dx, dy, dz, heading]

    Returns:
        mask: (N,) 布尔数组，表示每个点是否在任意框内
    """
    #print(f"points shape: {points.shape}")
    #print(f"boxes shape: {boxes.shape}")
    #print(f"boxes ndim: {boxes.ndim}")
    ...
    if boxes.ndim == 1:
        ...
        #print(f"Reshaped boxes shape: {boxes.shape}")
    ...
    for i in range(len(boxes)):
        box_mask = in_hull(points, corners[i])
        mask = mask | box_mask
