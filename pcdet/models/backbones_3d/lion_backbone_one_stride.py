    ...
    def forward(self, x, coords_shift=1, diffusion_scale=4):
        ...
        if self.diffusion:
            ...
            for i in range(batch_size):
                ...
                # 1. 创建当前样本的掩码
                mask = x.indices[:, 0] == i # 布尔掩码，形状 (N,)，标记属于第i个样本的体素
                valid_num = mask.sum() # 当前样本的体素数，标量

                # 2. 确定要扩散的体素数量 K
                K = int(valid_num * self.diff_scale) # diff_scale 是一个配置参数

                # 3. 选取Top-K重要体素
                # 对当前样本的所有体素，根据 x_feat_att 进行排序，取前K个最重要的
                _, indices = torch.topk(x_feat_att[mask], K)

                # 4. 获取这些重要体素的坐标和特征
                selected_coords_copy = x.indices[mask][indices].clone() # 坐标，形状 (K, 4)
                selected_coords_num = selected_coords_copy.shape[0] # K
                # 将坐标复制 diffusion_scale 份（通常是2或4）
                ...
                # 6. 修改复制出的坐标，实现“扩散”
                # 对每一份复制出的坐标，进行不同的偏移
                # 偏移量由 coords_shift 控制（例如1）
                # 第一份复制：X坐标减shift，Y坐标加shift
                ...
                selected_coords_expand[selected_coords_num * 0:selected_coords_num * 1, 1:2] = (
                        selected_coords_copy[:, 1:2]).clamp(min=0, max=d - 1) # Z坐标不变

                # 第二份复制：X坐标加shift，Y坐标加shift
                ...
                # 如果 diffusion_scale == 4，则还有两份复制
                if diffusion_scale==4:
                    # 第三份复制：X坐标减shift，Y坐标减shift
            ...
            # 将列表中所有批次的坐标和特征在0维（批次维度）拼接起来
            #  N_total = N_original + N_diffused。N_diffused 是扩散生成的虚拟体素总数
            coords = torch.cat(selected_diffusion_coords_list) # 形状 (N_total, 4)
            final_diffusion_feats = torch.cat(selected_diffusion_feats_list) # 形状 (N_total, C)
